...
node packages/cli/dist/bin.js mcp-config        # 별칭: karax mcp install-config
```
...
karax capture <path>                     전체 화면 캡처
  ...
  --out <dir>                          출력 디렉토리 (기본: /tmp/karax-out)
  --seed <n>                           결정론적 mock 시드
...
karax test <path>                        LLM 에이전트 E2E 테스트 (실기기 빌드·설치)
  ...
  --out <dir>                          결과 출력 디렉토리 (기본: /tmp/karax-e2e-out)
  --timeout <ms>                       에이전트 전체 타임아웃 (기본: 900000)
  --max-steps <n>                      에이전트 최대 스텝 수 (기본: 20)
  --keep-booted                        테스트 후 디바이스 유지
  --reuse-build                        소스 핑거프린트 일치 시 이전 빌드 재사용
  --no-build                           빌드 없이 캐시 artifact만 사용 (없으면 에러)
  --grant-permissions                  시나리오 permissions 자동 grant
  --record-video                       세션 비디오 녹화
  --no-fail-on-crash                   크래시 감지 시 fail 강등 비활성화
  --build-command <cmd>                기본 빌드 커맨드 대신 실행할 셸 커맨드
karax ui dump --device <id>              에이전트용 UI 헬퍼 — 현재 화면 요소 덤프
karax ui locate --device <id> --label <text>   텍스트/role로 요소 좌표 즉답
karax ui which-screen --device <id>      현재 화면 AppMap ID 매칭
karax mcp-config                         MCP 클라이언트 설정 스니펫 출력 (별칭: karax mcp install-config)
```

모든 커맨드는 `--debug`를 지원한다 ([디버그 모드](#디버그-모드) 참고).
...
- **iOS 입력**: idb가 설치된 경우 tap/swipe/text 주입. 없으면 좌표 추정 기반 fallback.
...
> `run_e2e_test` MCP 툴은 빌드·부팅을 포함하므로 **수 분이 소요**될 수 있다. `buildCommand` 파라미터로 기본 빌드 커맨드를 재정의할 수 있다 (예: `"fvm flutter build apk --debug --flavor dev"`).
>
> **iOS + `--build-command`**: karax가 `KARAX_DERIVED_DATA_PATH`(임시 디렉토리)를 빌드 환경에 주입한다. 커맨드에 `-derivedDataPath "$KARAX_DERIVED_DATA_PATH"`를 포함하면 karax가 `.app` 산출물을 확실히 찾을 수 있다. 없으면 `build/ios/iphonesimulator`·`ios/build/Build/Products` 순으로 fallback 탐색한다.

### `karax ui` — 에이전트용 결정론 헬퍼

에이전트가 E2E 테스트 중 현재 화면 상태를 결정론적으로 조회하는 서브커맨드. uiautomator(Android) / idb(iOS) 출력과 AppMap을 런타임 매칭한다.
...
# 텍스트/role로 요소 좌표 즉답
karax ui locate --device emulator-5554 --label "로그인"

# 현재 화면의 AppMap ID 매칭
karax ui which-screen --device emulator-5554 --appmap ./appmap.json
```

공통 플래그: `--device <id>` (필수), `--platform android|ios` (기본: android). `locate`는 추가로 `--label <text>` / `--appmap <path>` / `--screen <id>`를 받는다.
...
| **E2E 테스트 (LLM 에이전트)** | `test` | `run_e2e_test` | `runE2eTest` / `runE2eSuite` |
...
모든 CLI 커맨드는 `--debug` 플래그를 지원하며, `KARAX_DEBUG` 환경변수로도 켤 수 있다. 디버그 모드에서는 에러에 전체 스택 트레이스가 포함되고, 중간 아티팩트가 보존되어 원인 추적이 쉬워진다. 플래그/환경변수는 자식 프로세스로 자동 전파된다.
...
pnpm -r build             # 전체 빌드
pnpm test                 # 전체 테스트 (패키지 + scripts 런처 테스트)
...
# 골든/스냅샷 이미지 갱신 (명시적 리뷰 후에만 — 자동 갱신 금지)
...
아키텍처 의사결정의 배경은 자기완결적 설계 문서인 `PLAN.md`를 참고할 것.
