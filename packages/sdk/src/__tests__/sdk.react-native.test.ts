/**
 * SDK 통합 테스트 — react-native-basic fixture
 *
 * 별도 파일로 분리한 이유:
...
const RN_FIXTURE = path.resolve(process.cwd(), "../../fixtures/react-native-basic");

describe("react-native-basic fixture — detectFramework + listScreens + captureAll", () => {
  it("detectFramework: react-native가 1순위", async () => {
    const result = await detectFramework(RN_FIXTURE);
    expect(result.frameworks.length).toBeGreaterThan(0);
    expect(result.frameworks[0].id).toBe("react-native");
