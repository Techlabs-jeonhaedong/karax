/**
 * SDK 통합 테스트 — ios-swiftui-basic fixture
 *
 * 별도 파일로 분리한 이유:
...
const IOS_FIXTURE = path.resolve(process.cwd(), "../../fixtures/ios-swiftui-basic");

describe("ios-swiftui-basic fixture — detectFramework + listScreens + captureAll", () => {
  it("detectFramework: ios가 1순위", async () => {
    const result = await detectFramework(IOS_FIXTURE);
    expect(result.frameworks.length).toBeGreaterThan(0);
    expect(result.frameworks[0].id).toBe("ios");
