/**
 * React Native 화면별 골든 이미지 테스트 (Tier 2)
...
const TMP_DIR = resolve(__dirname, "__rn_golden_tmp__");
...
const FIXTURE_PATH = resolve(__dirname, "../../../..", "fixtures/react-native-basic");
const CTX = {
...
describe("React Native 화면 골든 이미지 (Tier 2)", () => {
  for (const { id, device } of SCREENS) {
    ...
    it(`${id} @ ${device}`, async () => {
      ...
      // IR 빌드
      const doc = await reactNativeAdapter.buildScreenIR(CTX, id);
      ...
      if (!existsSync(goldenPath)) {
        throw new Error(
          ...
          `생성하려면: UPDATE_GOLDENS=1 pnpm test --filter @karax/adapter-react-native`
        );
      }
