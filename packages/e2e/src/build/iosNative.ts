...
import { extractIosBundleId, findDerivedDataApp, findIosStandardApp } from "./artifact.js";
...
export class IosNativeBuilder implements AppBuilder {
  ...
  async build(projectPath: string, ctx?: BuildContext): Promise<BuildResult> {
    ...
    if (ctx?.buildCommand) {
      // 사용자 정의 빌드 커맨드: shell=true로 실행 (xcodebuild -list 스킵)
    ...
    if (!appPath) {
      const hint = ctx?.buildCommand
        ...
        : "";
      throw new E2eError("ARTIFACT_NOT_FOUND", `iOS .app을 찾을 수 없습니다.${hint}`);
    }
