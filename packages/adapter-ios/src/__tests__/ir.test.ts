...
describe("buildSwiftScreenIR — DetailScreen (커스텀 컴포넌트 인라이닝)", () => {
  ...
  it("PriceTag 인라이닝 — simple_identifier 체인 바인딩으로 구조 전달 (M1)", () => {
    ...
    // 'originalPrice'는 simple_identifier — 상위 ProductCard argBindings에서 체인 바인딩됨.
    // DetailScreen→ForEach에서 product.originalPrice는 member-access라 추출 불가이므로 mock 사용.
  ...
  it("가격 리터럴(299.99 등)은 ForEach member-access 구조로 인해 IR에 존재하지 않는다 — DYNAMIC_DATA_MOCKED 5건이 정상", () => {
    // DetailScreen은 private let products = [ProductItem(...)] 배열을 ForEach로 순회한다.
    // ProductCard(originalPrice: product.originalPrice, ...)에서
    // product.originalPrice는 navigation_expression(member-access)이라
    // 정적 AST 추적으로는 299.99/199.99/149.00/89.99/69.99 리터럴에 도달할 수 없다.
    //
    // 이 테스트는 "가격 리터럴 추출 불가" 사실을 명시적으로 단언함으로써
    // 미래에 이 기능이 구현됐을 때 테스트를 업데이트하도록 강제한다.
    //
    // DYNAMIC_DATA_MOCKED 수: product 인자 6개(title/description/imageName/originalPrice/discountedPrice/badge)
    // 중 member-access 인 것들. 최소 3개 이상 발생해야 함(originalPrice/discountedPrice/badge가 member-access).
    const mockedDiags = detailDoc.diagnostics?.filter(
      (d) => d.code === "DYNAMIC_DATA_MOCKED"
    ) ?? [];
    expect(mockedDiags.length).toBeGreaterThanOrEqual(3);

    // 실제 가격 리터럴이 Text 노드의 text.value에 존재하지 않음을 검증
    ...
    const pricePatterns = ["299.99", "199.99", "149.00", "149", "89.99", "69.99"];
    const foundPriceLiterals = pricePatterns.filter((p) =>
      textValues.some((v) => v.includes(p))
    );
    // 정적 추적 한계: 가격 리터럴은 IR에 없어야 한다
    // 만약 이 단언이 실패한다면 → member-access 추적 구현이 개선된 것이므로 테스트 업데이트 필요
    expect(foundPriceLiterals).toHaveLength(0);
  });
});
